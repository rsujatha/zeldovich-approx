...
class initial_density_field(lb.cosmology):
	...
	def var(self):
		...
		return Pk_discrete_half[rev_ind].reshape([self.GridSize,self.GridSize,int(self.GridSize/2)+1])
		
	def initial_deltak(self):
		mean=0
		np.random.set_state(self.rand_state)
		variance = initial_density_field.var(self)
		deltak = np.random.normal(mean,np.sqrt(variance), [self.GridSize,self.GridSize,int(self.GridSize/2)+1]) + np.random.normal(mean,np.sqrt(variance), [self.GridSize,self.GridSize,int(self.GridSize/2)+1])*1j
		deltak[0,0,0]=0
...
class LPT(initial_density_field,intpol.pmInterpolation):
	...
	def psik_scalar(self):
		...
		if self.deltax is None:
			...
			deltak = 1/self.GridSize**3 * np.fft.rfftn(deltax0)
			if self.Poisson_Solver ==0:
				kernel = -1/(ksquare + 1e-15)
				psik_scalar = kernel * deltak
				psik_scalar[0,0,0] = 0
	...
	def tidal_tensor(self,psik_scalar=None,k_x=None,k_y=None,k_z=None,flag=None):
		...
		if flag==0:
			...
			TidalTensor[:,1,2] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_y * k_z )).reshape([self.GridSize**3])
			...
			TidalTensor[:,:,:,0,0] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_x * k_x ))
			TidalTensor[:,:,:,0,1] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_x * k_y ))
			TidalTensor[:,:,:,0,2] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_x * k_z ))
			TidalTensor[:,:,:,1,0] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_y * k_x ))
			TidalTensor[:,:,:,1,1] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_y * k_y ))
			TidalTensor[:,:,:,1,2] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_y * k_z ))
			TidalTensor[:,:,:,2,0] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_z * k_x ))
			TidalTensor[:,:,:,2,1] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_z * k_y ))
			TidalTensor[:,:,:,2,2] = (self.GridSize**3 * np.fft.irfftn( - psik_scalar * k_z * k_z ))
		return TidalTensor
	...
	def deltax_zeld_ngp(self,RedShift,GridSize_ngp=None):
		if GridSize_ngp is None:
			GridSize_ngp = self.GridSize
		Position = LPT.Position_zeld(self,RedShift)
		PositionX = Position[0,:,:,:].flatten()
		PositionY = Position[1,:,:,:].flatten()
		PositionZ = Position[2,:,:,:].flatten()
		deltaxf = LPT.ngp(self,PositionX,PositionY,PositionZ,self.XSize,GridSize_ngp)*(GridSize_ngp/self.GridSize)**3-1
		return deltaxf
	...
	def PS_zeld_ngp(self,RedShift,GridSize_ngp=None,NofBins = 100,kmin=None, kmax=None):
		"""
		returns Pk , k in h Mpc^-1
		"""
		deltax = LPT.deltax_zeld_ngp(self,RedShift,GridSize_ngp)
		Pk,k = LPT.PS_calc(self,self.XSize,deltax,NofBins , kmin, kmax)
		return Pk,k
	...
	def psi2_vector0(self):
		...
		psi2k_scalar = - np.fft.rfftn(Lapsi2)/(ksquare+1e-15)
		psi2_vector_x0 = np.fft.irfftn(1j*k_x*psi2k_scalar) 
		psi2_vector_y0 = np.fft.irfftn(1j*k_y*psi2k_scalar) 
		psi2_vector_z0 = np.fft.irfftn(1j*k_z*psi2k_scalar) 
